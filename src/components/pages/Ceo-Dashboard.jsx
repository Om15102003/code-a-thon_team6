...
import { Search, Users, Upload, CheckSquare, User, Plus, X, BarChart2 } from 'lucide-react';
...
const ManagerDashboard = () => {
  ...
  const handleRunImpactAnalysis = async () => {
    try {
        ...
        const response = await fetch('http://localhost:4000/execute-impact-analysis', {
            method: 'POST',
        });

        if (!response.ok) {
            throw new Error('Failed to execute impact analysis');
        }

        const data = await response.json();
        alert('Impact analysis completed successfully! Check the generated Excel file.');
    } catch (error) {
      ...
      <div className="dashboard-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          >
            ...
            {analysisLoading ? 'Running Analysis...' : 'Run Impact Analysis'}
          </button>
        </nav>
